...
import { 
  ...
  Edit2, History, Moon, Sun, PieChart, LogOut, Lock, Mail, 
  Loader2, X, Search, ShieldAlert, Zap, Clock, CheckSquare, Square, Gift, Bot, Send
} from 'lucide-react';
...
// --- DOMAIN KNOWLEDGE: DISCOUNT CLUBS ---
const CLUBS = {
...
const DISCOUNTS_DATA = [
  // Behatsdaa
  { m: "Domino's Pizza (דומינוס פיצה)", c: "BEHATSDAA", d: "קופון 20% הנחה באפליקציה" },
  { m: "Pizza Hut (פיצה האט)", c: "BEHATSDAA", d: "קופון 20% הנחה" },
  { m: "Pizza Shemesh (פיצה שמש)", c: "BEHATSDAA", d: "קופון הנחה" },
  { m: "Papa John's (פאפא ג'ונס)", c: "BEHATSDAA", d: "קופון הנחה" },
  { m: "Pizza Prego (פיצה פרגו)", c: "BEHATSDAA", d: "קופון הנחה" },
  { m: "Cinema City (סינמה סיטי)", c: "BEHATSDAA", d: "כרטיס ב-20 ש\"ח" },
  { m: "Planet (פלאנט)", c: "BEHATSDAA", d: "כרטיס ב-20 ש\"ח" },
  { m: "HOT Cinema (הוט סינמה)", c: "BEHATSDAA", d: "כרטיס ב-20 ש\"ח" },
  { m: "Movieland (מובילנד)", c: "BEHATSDAA", d: "כרטיס ב-20 ש\"ח" },
  { m: "Mishloha (משלוחה)", c: "BEHATSDAA", d: "קופון 15% הנחה" },
  { m: "FOX (פוקס)", c: "BEHATSDAA", d: "תו קנייה ב-25% הנחה" },
  { m: "Mega Sport (מגה ספורט)", c: "BEHATSDAA", d: "תו קנייה ב-20% הנחה" },
  { m: "Aluf Sport (אלוף ספורט)", c: "BEHATSDAA", d: "תו קנייה ב-20% הנחה" },
  { m: "Holmes Place (הולמס פלייס)", c: "BEHATSDAA", d: "הנחות על מנויים שנתיים" },
...
// --- DOMAIN KNOWLEDGE: MERCHANT DATABASE ---
const KNOWN_MERCHANTS = {
  ...
  "Yango Deli (יאנגו דלי)": { cat: "Online Retail & Delivery", networks: ['CB'], aliases: ["יאנגו דלי", "יאנגו"] },
...
// --- FIX: AI ASSISTANT ENGINE (BACKEND READY + RATE LIMIT PROOF) ---
const fetchGeminiAIResponse = async (query, history, systemInstruction, signal) => {
  // APPROACH 1: THE BACKEND PROXY (The "Akim" App Method)
  // This is how you prevent 429 limits and secure your key.
  // When you host this on Netlify/Vercel later, this will route the request to your backend.
  try {
      ...
      // It's perfectly fine if this fails during local testing.
      console.log("Backend proxy not found. Falling back to direct client API...");
  }

  // APPROACH 2: DIRECT CLIENT FALLBACK (For testing in Canvas)
  // We use your exact key and limit history heavily to avoid tokens limits.
  const apiKey = "AIzaSyAO8DMcE26LIT7zCHjV1RioVpMOKWfZ24U";
  const model = "gemini-2.0-flash";
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`;

  // AGGRESSIVE TRIM: Only send the latest query to avoid 429 Payload Limit
  const payload = {
  ...
  try {
      ...
      if (!response.ok) {
          if (response.status === 429) {
              throw new Error("429: Too Many Requests. הגענו למגבלת הפניות החינמית של גוגל. כדי לעקוף זאת לחלוטין, יש להעלות את האתר עם שרת אחורי (Netlify Functions) כפי שמוגדר כעת בקוד.");
          }
      ...
      return result.candidates?.[0]?.content?.parts?.[0]?.text || "לא בטוח איך לענות על זה כרגע.";
  } catch (error) {
...
export default function App() {
  ...
  const handleSendAI = async (e) => {
    ...
    const systemInstruction = `You are a smart, witty, and highly practical Israeli shopping assistant.
Always match the user's language (if they write in English, reply in English. If Hebrew, reply in Hebrew).
Your tone should be friendly, slightly humorous, but highly structured and direct.
...
RESPONSE RULES:
1. Don't be generic. Identify exactly what the user wants to buy.
2. Recommend specific merchants from the 'Supported Merchants' list.
3. Explicitly state WHICH specific card from their 'Wallet' they should use based on their available balances. (e.g. "Use your HappyGift card since you have ₪500 on it").
4. If a relevant discount exists in 'Available Discounts', HIGHLIGHT IT strongly!
5. Format your response clearly using bullet points and bold text for readability.
6. If their wallet can't cover the full estimated amount, suggest splitting the payment.
7. Add a touch of friendly Israeli sass/humor (e.g., emojis, "יאללה", "תהנה").`;
  ...
  const handleMerchantSelect = (name, cat) => {
    setMerchantSearch(name); setNewExpense({ ...newExpense, merchantName: name, category: cat, cardId: '' }); setShowMerchantSuggestions(false);
  };
      ...
      <div className={`min-h-screen ${isDarkMode ? 'dark' : ''}`}>
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4 transition-colors duration-300">
          <div className="mb-10 text-center animate-in slide-in-from-bottom-4 fade-in duration-500">
            <div className="bg-gradient-to-tr from-blue-600 to-indigo-600 p-5 rounded-3xl shadow-xl mb-6 inline-block">
          ...
          <div className="w-full max-w-md bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 animate-in slide-in-from-bottom-8 fade-in duration-700 delay-150">
            ...
            <div className="mt-8 text-center text-sm text-slate-500 dark:text-slate-400">
              <button type="button" onClick={() => { setIsLoginMode(!isLoginMode); setAuthError(''); }} className="text-blue-600 dark:text-blue-400 font-bold hover:underline">
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 transition-colors duration-300 relative">
        ...
        <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 p-4 sm:p-5 sticky top-0 z-40 transition-colors">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="bg-gradient-to-tr from-blue-600 to-indigo-600 p-2 sm:p-2.5 rounded-xl shadow-lg">
                <CreditCard size={24} className="text-white" />
              </div>
              <div>
                <h1 className="text-lg sm:text-xl font-extrabold tracking-tight text-slate-900 dark:text-white">CardsDeVen</h1>
                <p className="text-[10px] sm:text-xs font-medium text-slate-500 dark:text-slate-400">{user.email}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2.5 bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-slate-600 dark:text-slate-300">
                {isDarkMode ? <Sun size={18} /> : <Moon size={18} />}
              </button>
              <button onClick={handleSignOut} className="p-2.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-full hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
                <LogOut size={18} />
              </button>
            </div>
          </div>
        ...
        <main className="max-w-6xl mx-auto p-4 sm:p-6 mt-2 sm:mt-6">
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {/* FUNCTIONAL 2: Urgency Dashboard */}
              {expiringAlerts.length > 0 && (
                <div className="bg-gradient-to-r from-orange-500 to-red-600 rounded-[2rem] p-6 shadow-lg text-white flex flex-col sm:flex-row items-center gap-4 sm:justify-between animate-in zoom-in-95">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-lg">Use It Or Lose It</h3>
                      <p className="text-white/80 text-sm">{expiringAlerts.length} card(s) expiring within 30 days!</p>
                    </div>
                  </div>
                    ...
                    {expiringAlerts.map(card => (
                      <div key={card.id} className="bg-black/20 px-4 py-2 rounded-xl flex justify-between items-center gap-6 backdrop-blur-md">
                        ...
                        <div className="text-right">
                          <div className="font-black">₪{card.remaining.toLocaleString()}</div>
                          <div className="text-[10px] uppercase tracking-wider text-orange-200">In {getDaysUntilExpiry(card.expiryDate)} Days</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-700 text-white p-8 rounded-[2rem] shadow-xl relative overflow-hidden group hover:shadow-2xl transition-all">
                  <div className="relative z-10">
                    <div className="text-slate-300 dark:text-slate-400 text-sm font-semibold mb-2 uppercase tracking-widest">Total Portfolio</div>
                    <div className="text-5xl font-black mb-1">₪{totalInitialBalance.toLocaleString()}</div>
                    <div className="text-slate-400 dark:text-slate-500 text-sm">Initial setup across {cards.length} cards</div>
                  </div>
                  <div className="absolute -right-8 -bottom-8 bg-white/5 p-8 rounded-full group-hover:scale-110 transition-transform duration-500">
                    <CreditCard size={100} className="text-white/10" />
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-emerald-500 to-emerald-700 text-white p-8 rounded-[2rem] shadow-xl relative overflow-hidden group hover:shadow-2xl transition-all">
                  <div className="relative z-10">
                    <div className="text-emerald-100 text-sm font-semibold mb-2 uppercase tracking-widest">Available Power</div>
                    <div className="text-5xl font-black mb-1">₪{totalRemainingBalance.toLocaleString()}</div>
                    <div className="text-emerald-200 text-sm">After ₪{totalPlannedExpenses.toLocaleString()} total expenses</div>
                  </div>
                  <div className="absolute -right-8 -bottom-8 bg-black/5 p-8 rounded-full group-hover:scale-110 transition-transform duration-500">
                    <Receipt size={100} className="text-black/10" />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                {fundsByCategory.length === 0 ? (
                  <div className="text-center p-12 bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800">
                    <PieChart size={48} className="mx-auto mb-4 text-slate-300 dark:text-slate-700" />
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Add cards to populate your category breakdown.</p>
                  </div>
                ) : (
                    ...
                    {fundsByCategory.map(([category, data]) => (
                      <div key={category} className="p-6 bg-white dark:bg-slate-900 rounded-[1.5rem] shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-md hover:-translate-y-1 transition-all group">
                        ...
                        <div className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                          <span className="font-semibold">Sources:</span> {data.sources.join(', ')}
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'wallets' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex justify-between items-end">
                <div>
                  <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Digital Wallet</h2>
                  <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your active gift cards and budgets.</p>
                </div>
                <button onClick={() => { resetCardForm(); setShowCardForm(true); }} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-5 py-3 rounded-xl font-bold flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-lg hover:shadow-xl">
                  <Plus size={20} /> Add Card
                </button>
              </div>

              {cards.length === 0 ? (
                <div className="text-center p-16 bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 border-dashed">
                  <CreditCard size={64} className="mx-auto mb-6 text-slate-200 dark:text-slate-800" />
                  <h3 className="text-xl font-bold text-slate-700 dark:text-slate-200 mb-2">Your wallet is empty</h3>
                  <p className="text-slate-500 dark:text-slate-400 max-w-sm mx-auto">Click the button above to register your first funding source or gift card.</p>
                </div>
              ) : (
                      ...
                      <div key={card.id} className="relative group perspective-1000">
                          ...
                          <div className="flex justify-between items-start relative z-10">
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="bg-white/20 px-2 py-0.5 rounded text-[10px] font-bold tracking-widest uppercase backdrop-blur-md border border-white/20">{progData.name}</span>
                              </div>
                            ...
                            <div className="flex flex-col items-end gap-2">
                              {/* UX 2: Quick Spend Button - Always visible on mobile */}
                              <button onClick={() => startQuickExpense(card.id)} className="bg-white text-slate-900 hover:bg-emerald-400 hover:text-white p-2.5 rounded-full shadow-lg transition-colors flex items-center gap-2 group/btn">
                                <Zap size={18} className="fill-current" />
                                <span className="hidden sm:group-hover/btn:block text-xs font-bold uppercase tracking-wider pr-2">Quick Spend</span>
                              </button>
                        ...
                        <div className="px-4 py-4 flex flex-wrap gap-2">
                          {card.derivedCats.map(cat => (
                            <span key={cat} className="bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm flex items-center gap-1.5">
          ...
          {activeTab === 'planner' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex justify-between items-end">
                <div>
                  <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Expense Planner</h2>
                  <p className="text-slate-500 dark:text-slate-400 mt-1">Plan and verify purchases against your wallet rules.</p>
                </div>
                <button onClick={() => { resetExpenseForm(); setShowExpenseForm(true); }} className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-3 rounded-xl font-bold flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-lg hover:shadow-xl">
                  <Plus size={20} /> Plan Purchase
                </button>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex justify-between items-center">
                  <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100">Ledger</h3>
                  <span className="bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-4 py-1.5 rounded-full text-sm font-bold">
                    Total: ₪{totalPlannedExpenses.toLocaleString()}
                  </span>
                </div>
                {expenses.length === 0 ? (
                  <div className="p-16 text-center text-slate-500 dark:text-slate-400 flex flex-col items-center">
                    <Receipt size={48} className="text-slate-200 dark:text-slate-800 mb-4" />
                    <p className="font-medium text-lg">No purchases planned yet.</p>
                    <p className="text-sm">Plan a purchase to reserve funds.</p>
                  </div>
                ) : (
                  <div className="divide-y divide-slate-100 dark:divide-slate-800">
                    {sortedExpenses.map(expense => {
                      ...
                      // FUNCTIONAL 3: Planned vs Completed UI
                      return (
                        <div key={expense.id} className={`p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors group ${expense.isCompleted ? 'bg-emerald-50/30 dark:bg-emerald-900/10' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                          <div className="flex items-center gap-4 sm:gap-5">
                            ...
                            <button onClick={() => toggleExpenseCompletion(expense)} className={`w-12 h-12 sm:w-14 sm:h-14 shrink-0 rounded-2xl flex flex-col items-center justify-center transition-all shadow-sm ${expense.isCompleted ? 'bg-emerald-500 text-white' : 'bg-white dark:bg-slate-800 border-2 border-dashed border-slate-300 dark:border-slate-600 text-slate-400 hover:border-emerald-400 hover:text-emerald-500'}`}>
                              {expense.isCompleted ? <CheckSquare size={20} className="sm:w-6 sm:h-6" /> : <Square size={20} className="sm:w-6 sm:h-6" />}
                              <span className="text-[8px] sm:text-[9px] font-bold uppercase mt-0.5">{expense.isCompleted ? 'Paid' : 'Plan'}</span>
                            </button>
                            ...
                            <div>
                              <div className={`font-bold text-base sm:text-lg mb-1 flex flex-wrap items-center gap-2 ${expense.isCompleted ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                                ...
                                {expense.merchantName && (
                                  <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[9px] sm:text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wider flex items-center gap-1">
                                    <MerchantIcon merchantName={expense.merchantName} category={expense.category} className="w-4 h-4 rounded-sm border-0" />
                                    {expense.merchantName.split('(')[0].trim()}
                                  </span>
                                )}
                              </div>
                              <div className="flex flex-wrap items-center gap-2 text-xs">
                                <span className="font-medium text-slate-500">{CATEGORY_ICONS[expense.category]} {expense.category}</span>
                                <span className="text-slate-300 dark:text-slate-600">•</span>
                                <span className="font-bold flex items-center gap-1">
                                  <span className={`w-2 h-2 rounded-full ${progColor}`}></span>
                                  {sourceCard?.name || 'Deleted Card'}
                                </span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto pl-16 sm:pl-0">
                            <div className={`font-black text-xl sm:text-2xl ${expense.isCompleted ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-400'}`}>
          ...
          {activeTab === 'insights' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div>
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Smart Merchant Search</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Find out exactly which cards & discounts to use at the checkout counter.</p>
              </div>
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-[2rem] p-6 sm:p-8 shadow-xl text-white">
                ...
                <div className="relative w-full">
                  <input 
                    ...
                    placeholder="e.g. Zara, Pizza, Cinema, ASOS..." 
                    className="w-full pl-5 pr-12 py-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-white placeholder-white/60 focus:ring-4 focus:ring-white/30 outline-none font-medium text-lg transition-all"
                  />
                </div>
                ...
                {insightSearch && (
                  <div className="mt-6 space-y-4">
                    {(() => {
                      ...
                      if (matches.length === 0) {
                        return (
                          <div className="text-white/80 font-medium bg-white/10 p-4 rounded-2xl border border-white/20">
                            Merchant not found in official database. Generic category rules will apply.
                          </div>
                        );
                      }
                        ...
                        return (
                          <div key={searchMatch} className="animate-in slide-in-from-bottom-2 fade-in bg-white/10 p-5 rounded-2xl border border-white/20 shadow-md">
                            <div className="flex items-center gap-3 mb-4">
                              {/* UX 3: Dynamic Image Loading with Emoji Fallback */}
                              <MerchantIcon merchantName={searchMatch} category={mData.cat} className="w-10 h-10 rounded-full" />
                              <div>
                                <div className="text-base sm:text-lg font-bold text-white leading-tight">{searchMatch}</div>
                                <div className="text-[10px] sm:text-xs uppercase tracking-widest text-blue-200 mt-0.5">{CATEGORY_ICONS[mData.cat]} {mData.cat}</div>
                              </div>
                            </div>
                            ...
                            {acceptedCards.length > 0 ? (
                              <div className="flex flex-wrap gap-2 sm:gap-3">
                                {acceptedCards.map(c => {
                                  ...
                                  return (
                                    <div key={c.id} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl font-bold shadow-md text-sm ${isExpiringSoon ? 'bg-orange-100 text-orange-900 border-2 border-orange-500' : 'bg-white text-slate-900'}`}>
                                      {isExpiringSoon ? <Clock size={16} className="text-orange-600" /> : <CheckCircle2 size={16} className="text-emerald-500" />} 
                                      <span className="truncate max-w-[100px] sm:max-w-none">{c.name}</span>
                                      <span className={`${isExpiringSoon ? 'bg-orange-200 text-orange-900' : 'bg-emerald-100 text-emerald-800'} px-1.5 py-0.5 rounded text-xs ml-0.5`}>
                              ...
                              <div className="bg-red-500/20 border border-red-500/50 text-white px-4 py-3 rounded-xl inline-flex items-start sm:items-center gap-2 font-medium text-sm sm:text-base">
                                <AlertCircle size={18} className="shrink-0 mt-0.5 sm:mt-0" /> 
                                <span>No active cards have funds for this merchant.</span>
                              </div>
                            )}
                            ...
                            {merchantDeals.length > 0 && (
                              <div className="mt-4 pt-4 border-t border-white/20">
                                <div className="text-[10px] font-bold uppercase tracking-wider text-amber-200 mb-3 flex items-center gap-1.5">
                                  <Gift size={14} /> Club Deals Available
                                </div>
              ...
              {cards.length > 0 && (
                <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl border border-slate-100 dark:border-slate-800 overflow-hidden">
                  <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
                    <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100">Broad Category Coverage</h3>
                  </div>
                  
                  {/* Mobile Friendly Matrix Table */}
                  <div className="p-4 sm:hidden space-y-4">
                    {[...new Set(cardBalances.flatMap(c => c.derivedCats || []))].sort().map(category => {
                      const coveringCards = cardBalances.filter(c => (c.derivedCats || []).includes(category));
                      return (
                        <div key={category} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700/50">
                          <h4 className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2 mb-3 border-b border-slate-200 dark:border-slate-700 pb-2">
                            <span>{CATEGORY_ICONS[category]}</span> {category}
                          </h4>
                          <div className="flex flex-wrap gap-2">
                            {coveringCards.length > 0 ? (
                              coveringCards.map(card => (
                                <span key={card.id} className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 text-xs px-2.5 py-1 rounded-md font-bold">
                                  {card.name}
                                </span>
                              ))
                            ) : (
                              <span className="text-xs font-medium text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">No Coverage</span>
                            )}
                          </div>
                        </div>
                      )
                  ...
                  <div className="hidden sm:block overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                      <thead>
                        <tr>
                          <th className="p-6 text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest bg-slate-50 dark:bg-slate-900/50 sticky left-0 z-20 backdrop-blur-md">Coverage Areas</th>
                          {cardBalances.map(card => {
                            const progData = PROGRAMS[card.programId || 'CUSTOM'] || PROGRAMS.CUSTOM;
                            return (
                              <th key={card.id} className="p-6 text-center border-l border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 min-w-[150px]">
                                <div className={`${card.color || GRADIENTS[0]} w-8 h-8 rounded-full mx-auto mb-3 shadow-md`}></div>
                                <div className="font-bold text-slate-800 dark:text-slate-200 mb-1">{card.name}</div>
                                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{progData.name}</div>
                              </th>
                            )
                          })}
                        </tr>
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {[...new Set(cardBalances.flatMap(c => c.derivedCats || []))].sort().map(category => (
                          <tr key={category} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                            <td className="p-5 font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 sticky left-0 z-10 group-hover:bg-slate-50 dark:group-hover:bg-slate-800/50 transition-colors flex items-center gap-2">
                              <span>{CATEGORY_ICONS[category]}</span> {category}
                            </td>
                            {cardBalances.map(card => (
                              <td key={`${card.id}-${category}`} className="p-5 text-center border-l border-slate-100 dark:border-slate-800">
                                {(card.derivedCats || []).includes(category) 
                                  ? <CheckCircle2 size={24} className="text-emerald-500 mx-auto" /> 
                                  : <div className="w-2 h-2 rounded-full bg-slate-200 dark:bg-slate-800 mx-auto"></div>
                                }
                              </td>
                            ))}
                          </tr>
          ...
          {activeTab === 'clubs' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div>
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Discount Clubs</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Select your clubs to unlock exclusive deals.</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {Object.values(CLUBS).map(club => {
                  ...
                  return (
                    <button key={club.id} onClick={() => handleToggleClub(club.id)} className={`p-4 rounded-2xl border-2 transition-all flex items-center justify-between shadow-sm ${isActive ? `border-transparent ${club.color} text-white shadow-lg transform scale-105` : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-500 hover:border-slate-300 dark:hover:border-slate-600'}`}>
                      <span className="font-bold text-lg">{club.name}</span>
                      {isActive ? <CheckCircle2 size={24} /> : <Plus size={24} />}
                    </button>
                  )
                })}
              </div>
                ...
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
                  <div className="relative max-w-md">
                    ...
                    <input 
                      ...
                      placeholder="Search discounts (e.g. Pizza, FOX)..." 
                      className="w-full pl-10 p-3.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium" 
                    />
                  </div>
                </div>
                <div className="p-6">
                  {userClubs.length === 0 ? (
                    <div className="text-center p-8">
                      <Gift size={48} className="mx-auto mb-4 text-slate-200 dark:text-slate-800" />
                      <p className="text-slate-500 font-medium">Select a club above to see your available deals.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {DISCOUNTS_DATA.filter(d => {
                        ...
                        return d.m.toLowerCase().includes(qs) || 
                               d.d.toLowerCase().includes(qs) || 
                               cat.toLowerCase().includes(qs) || 
                               catAliases.some(a => a.includes(qs));
                      }).map((deal, idx) => (
                        <div key={idx} className="flex gap-4 items-start p-4 border border-slate-100 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                          ...
                          <div>
                            <div className="font-bold text-slate-900 dark:text-white flex items-center gap-2 mb-1">
                              {deal.m.split('(')[0].trim()}
                              <span className={`text-[9px] px-1.5 py-0.5 rounded text-white ${CLUBS[deal.c].color}`}>{CLUBS[deal.c].name}</span>
                            </div>
                            <div className="text-sm font-medium text-emerald-600 dark:text-emerald-400 leading-tight">{deal.d}</div>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          {activeTab === 'ai' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 h-[80vh] flex flex-col">
              <div className="flex justify-between items-end">
                <div>
                  <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Smart Assistant</h2>
                  <p className="text-slate-500 dark:text-slate-400 mt-1">Ask me what to buy, and I'll find the best deal.</p>
                </div>
                <button onClick={() => {
                  if (abortControllerRef.current) abortControllerRef.current.abort();
                  setAiMessages([{role: 'model', text: 'היסטוריית הצ\'אט נמחקה! אז מה קונים היום? 😎'}]);
                  setIsAiTyping(false);
                }} className="p-2.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-colors" title="Clear Chat History">
                  <Trash2 size={20} />
                </button>
              </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
                  ...
                  {isAiTyping && (
                    <div className="flex justify-start">
                      <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center shrink-0 mr-3 mt-1"><Bot size={16} className="text-blue-600 dark:text-blue-400" /></div>
                      <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl rounded-tl-sm border border-slate-100 dark:border-slate-700/50"><Loader2 className="animate-spin text-slate-400" size={20} /></div>
                    </div>
                  )}
                  <div ref={chatEndRef} />
                </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800">
                  <form onSubmit={handleSendAI} className="relative flex items-center">
                    <input 
                      ...
                      placeholder="e.g. I need to buy pizza for 10 people..."
                      disabled={isAiTyping}
                      className="w-full pl-4 pr-14 py-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all shadow-sm disabled:opacity-70"
                    />
                    <button type="submit" disabled={!aiInput.trim() || isAiTyping} className="absolute right-2 p-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors">
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] px-2 sm:px-6 py-4 flex justify-around sm:justify-center sm:gap-8 lg:gap-16 z-40 transition-colors overflow-x-auto">
          {[
            ...
            { id: 'wallets', icon: CreditCard, label: 'Wallet' },
            { id: 'planner', icon: Receipt, label: 'Planner' },
            { id: 'insights', icon: Search, label: 'Search' },
            { id: 'clubs', icon: Gift, label: 'Clubs' },
            { id: 'ai', icon: Bot, label: 'Smart AI' },
          ].map(item => (
            <button 
              key={item.id}
              onClick={() => setActiveTab(item.id)} 
              className={`flex flex-col items-center gap-1.5 transition-all duration-300 min-w-[50px] ${activeTab === item.id ? 'text-blue-600 dark:text-blue-400 scale-110' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:scale-105'}`}
            >
          ...
          <form onSubmit={handleSaveCard} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <input type="text" required value={newCard.name} onChange={e => setNewCard({...newCard, name: e.target.value})} className="w-full p-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium" placeholder="e.g. My Cibus Card" />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Total Limit (₪)</label>
                <input type="number" required min="0" step="0.01" value={newCard.balance} onChange={e => setNewCard({...newCard, balance: e.target.value})} className="w-full p-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-mono font-bold text-lg" placeholder="0.00" />
              </div>
            </div>
            ...
            {newCard.ruleType === 'expires' && (
              <div className="animate-in slide-in-from-top-2 fade-in">
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Expiration Date</label>
                <input type="date" required value={newCard.expiryDate || ''} onChange={e => setNewCard({...newCard, expiryDate: e.target.value})} className="w-full sm:w-1/2 p-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" />
              </div>
            )}
            ...
            {newCard.programId === 'CUSTOM' ? (
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl flex items-start gap-3">
                ...
                <div>
                  <div className="font-bold text-blue-800 dark:text-blue-300 text-sm">Auto-Managed Logic</div>
                  <div className="text-xs text-blue-600 dark:text-blue-400 mt-1">Categories and accepted merchants for {PROGRAMS[newCard.programId].name} are managed automatically by the system engine.</div>
                </div>
              </div>
            )}

            <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
              <button type="submit" disabled={newCard.programId === 'CUSTOM' && newCard.categories.length === 0} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl disabled:opacity-50 transition-all shadow-lg hover:shadow-xl active:scale-[0.98] text-lg">
                {editingCardId ? 'Update Wallet' : 'Add to Wallet'}
              </button>
            </div>
          </form>
        </Modal>
          ...
          <form onSubmit={handleSaveExpense} className="space-y-6">
            ...
            <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
              <input 
                type="checkbox" 
                id="isCompleted"
                checked={newExpense.isCompleted}
                onChange={e => setNewExpense({...newExpense, isCompleted: e.target.checked})}
                className="w-6 h-6 text-emerald-600 bg-white border-slate-300 rounded focus:ring-emerald-500"
              />
              <label htmlFor="isCompleted" className="cursor-pointer">
                <div className="font-bold text-slate-800 dark:text-slate-200">Already Spent?</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">Check this if you have already completed this purchase at the store.</div>
              </label>
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Item / Purpose</label>
              <input type="text" required value={newExpense.name} onChange={e => setNewExpense({...newExpense, name: e.target.value})} className="w-full p-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all font-medium" placeholder="e.g. Cinema Tickets" />
            </div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    onChange={e => {
                      ...
                      setNewExpense({...newExpense, merchantName: e.target.value, cardId: ''});
                      setShowMerchantSuggestions(true);
                    }}
                    onFocus={() => setShowMerchantSuggestions(true)}
                    className="w-full pl-10 p-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all font-medium" 
                    placeholder="e.g. Wolt, FOX..." 
                  />
                  ...
                  {showMerchantSuggestions && merchantSearch && (
                    <div className="absolute z-50 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl max-h-48 overflow-y-auto">
                      {getSmartMatches(merchantSearch).map(([name, data]) => (
                          <div key={name} onMouseDown={() => handleMerchantSelect(name, data.cat)} className="p-3 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer border-b border-slate-100 dark:border-slate-700/50 last:border-0 flex justify-between items-center">
                            <div className="flex items-center gap-2">
                              <MerchantIcon merchantName={name} category={data.cat} className="w-6 h-6 rounded border-0 bg-transparent" />
                      ...
                      {getSmartMatches(merchantSearch).length === 0 && (
                        <div className="p-3 text-sm text-slate-500 text-center">Unrecognized merchant. Using generic rules.</div>
                      )}
                    </div>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
               <div>
                ...
                {!editingExpenseId && newExpense.amount && (
                  <div className="mt-3 flex items-center gap-2">
                    <input 
                      type="checkbox" 
                      id="isManualSplit" 
                      checked={newExpense.isManualSplit || false} 
                      onChange={e => setNewExpense({...newExpense, isManualSplit: e.target.checked, chargeAmount: e.target.checked ? newExpense.amount : ''})} 
                      className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 bg-white border-slate-300" 
                    />
                    <label htmlFor="isManualSplit" className="text-xs font-semibold text-slate-500 dark:text-slate-400 cursor-pointer">
                      Split this payment across multiple cards?
                    </label>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <select required disabled={!newExpense.category} value={newExpense.cardId} onChange={e => setNewExpense({...newExpense, cardId: e.target.value})} className="w-full p-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 disabled:opacity-50 outline-none transition-all font-medium appearance-none">
                  ...
                  {sortedCardBalances.map(card => {
                    ...
                    return (
                      <option key={card.id} value={card.id} disabled={!isSelectable}>
                        {expiringTag}{card.name} (Available: ₪{card.remaining.toLocaleString()}) {!isAllowedByRules ? '- Rule Blocked' : (!canAfford ? '- Requires Split' : '')}
                      </option>
                    )
                  })}
                </select>
                {newExpense.category && cardBalances.filter(c => checkCompatibility(c, newExpense.category, newExpense.merchantName).allowed).length === 0 && (
                   <p className="text-red-500 dark:text-red-400 text-[10px] mt-1.5 font-bold uppercase tracking-wider flex items-center gap-1"><ShieldAlert size={12}/> No valid cards for this merchant/category.</p>
                )}
              </div>
            </div>
            ...
            {newExpense.isManualSplit && !editingExpenseId && newExpense.cardId && (
               <div className="animate-in fade-in slide-in-from-top-2 bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-100 dark:border-blue-800/50">
                 ...
                 <input 
                    ...
                    required 
                    min="0.01" 
                    max={Math.min(parseFloat(newExpense.amount || Infinity), cardBalances.find(c => c.id === newExpense.cardId)?.remaining || Infinity)} 
                    step="0.01" 
                    value={newExpense.chargeAmount} 
                    onChange={e => setNewExpense({...newExpense, chargeAmount: e.target.value})} 
                    className="w-full p-3.5 bg-white dark:bg-slate-800 border border-blue-200 dark:border-blue-700 text-slate-900 dark:text-white rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-mono font-bold text-lg" 
                    placeholder="0.00" 
                 />
               </div>
            )}
            ...
            <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
              {(() => {
                ...
                if (selectedCard && !editingExpenseId) {
                  if (newExpense.isManualSplit && newExpense.chargeAmount) {
                    actualLogAmount = parseFloat(newExpense.chargeAmount || 0);
                  }
                  if (actualLogAmount > selectedCard.remaining) {
                    actualLogAmount = selectedCard.remaining;
                  }
                  if (actualLogAmount < reqAmount && actualLogAmount > 0) {
                    isSplitNeeded = true;
                  }
                }

                return (
                  <button type="submit" className={`w-full text-white font-bold py-4 rounded-xl transition-all shadow-lg hover:shadow-xl active:scale-[0.98] text-lg ${isSplitNeeded ? 'bg-orange-500 hover:bg-orange-600' : 'bg-emerald-600 hover:bg-emerald-700'}`}>
                    {isSplitNeeded ? `Split Payment (Log ₪${actualLogAmount} & Continue)` : (editingExpenseId ? 'Update Purchase' : 'Confirm Plan')}
                  </button>
                )
              })()}
            </div>
